...
const ShareWith = ({ details }) => {
  ...
  const getBuddiesData = async () => {
    ...
    const buddies = res.data.buddies.filter((bud) => bud.buddyStatus == 'accepted')
  ...
  const selectBuddy = (selectedOptions) => {
    ...
    var newBuddy = selectedBuddies.filter((bud) => bud.id === selectedOptions.id)
  ...
  return (
    ...
    <Heading>Details</Heading>
      ...
      <Button
        ...
        color="#00A652"
        margin='20px 0 0 0'
        onClick={() => !isAddButtonShown ? setIsAddButtonShown(true) : addBuddiesToFilePermission()}
      >
