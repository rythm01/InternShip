...
export const fileController = {
    getFiles: async (req: Request, res: Response) => {
        ...
        try {
            const userProfile = await UserProfileRepo.createQueryBuilder("userProfile").innerJoinAndSelect("userProfile.userAuth", "UserAuth").where("userProfile.userAuth = :id", { id: req.user }).getOne();
            ...
            return res.json({ success: true, message: 'Files found successfully', data: files });
        }
        catch (error) {
            console.log(error)
    ...
    uploadFile: async (req: Request, res: Response) => {
        try {
            ...
            const rootFolder = await FolderRepo.findOne({ where: { name: 'root', user: { id: userProfile.id } } })
            if (!rootFolder) {
                return res.status(200).json({ success: false, message: 'Root folder does not exist' });
            }
    ...
    viewFile: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const file = await FileRepo.findOne({ where: { id: parseInt(id), user: { id: userProfile.id } } })
            ...
            return res.status(200).json({ success: true, data: { url, ext: file.ext } });
