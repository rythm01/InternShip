...
export const folderController = {
    ...
    updateFolder: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const folder = await FolderRepo.createQueryBuilder("folder").innerJoin("folder.user", "user").where("user.id = :id", { id: userProfile?.id }).andWhere("folder.id = :folderId", { folderId: id }).getOne();
    ...
    deleteFolder: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const findFolder = await FolderRepo.findOne({ where: { id: parseInt(req.params.id), user: { id: userProfile.id } } })
            if (!findFolder) {
                return res.status(200).json({ success: false, message: 'Folder does not exist' });
            }
