...
export const authController = {
    ...
    login: async (req: Request, res: Response) => {
        try {
            ...
            if (!user.is2fa) {
                const token = jwt.sign({ id: user.id, email: user.email }, process.env.JWT_SECRET || "hcjhad7842687ahdcb");
            ...
            twilio.messages.create({
                ...
                if (!message.sid) return res.status(200).json({ success: false, message: 'Something went wrong!' });
                var date = new Date();
    ...
    loginWithGoogle: async (req: Request, res: Response) => {
        try {
            ...
            var ticket = await client.verifyIdToken({ idToken: req.body.credential, audience: process.env.GOOGLE_CLIENT_ID });
    ...
    register: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const user = await UserRepo.findOne({ where: [{ phoneNumber }, { email }] });
    ...
    loginAdmin: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const token = jwt.sign({ id: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || "hcjhad7842687ahdcb");
            return res.json({ token, user, success: true, is2fa: false });
        }
        catch (error) {
            console.log(error);
    ...
    registerAdmin: async (req: Request, res: Response) => {
        ...
        try {
            ...
            const token = jwt.sign({ id: newUser.id, email: newUser.email, role: newEntry.role }, process.env.JWT_SECRET || "hcjhad7842687ahdcb");
